...
from gcloud.core.apis.drf.serilaziers.common_template import (
    ...
    UpdateCommonTemplateSerializer,
)
from gcloud.core.apis.drf.viewsets import DraftTemplateViewSetMixin
...
from gcloud.user_custom_config.constants import DEFAULT_PIPELINE_TREE
...
class CommonTemplatePermission(IamPermission):
    actions = {
        ...
        "draft": IamPermissionInfo(
            IAMMeta.COMMON_FLOW_VIEW_ACTION, res_factory.resources_for_common_flow_obj, HAS_OBJECT_PERMISSION
        ),
        "update_draft": IamPermissionInfo(
            IAMMeta.COMMON_FLOW_PUBLISH_DRAFT_ACTION, res_factory.resources_for_common_flow_obj, HAS_OBJECT_PERMISSION
        ),
...
class CommonTemplateViewSet(GcloudModelViewSet, DraftTemplateViewSetMixin):
    ...
    @swagger_auto_schema(method="GET", operation_summary="获取公共流程的草稿信息")
    @action(methods=["GET"], detail=True)
    ...
    @swagger_auto_schema(method="post", operation_summary="更新公共流程草稿信息", request_body=UpdateDraftPipelineTreeSerializer)
    ...
    def update_draft(self, request, *args, **kwargs):
        ...
        result = self.update_template_draft(manager, common_template, request)
        if not result["result"]:
            message = result["message"]
            logger.error(message)
            return Response({"detail": ErrorDetail(message, err_code.REQUEST_PARAM_INVALID.code)}, exception=True)
    ...
    @swagger_auto_schema(method="post", operation_summary="发布公共流程")
    @action(methods=["POST"], detail=True)
