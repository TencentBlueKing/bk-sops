...
class CommonProjectViewSet(GcloudListViewSet):
    ...
    @staticmethod
    def get_default_projects(username, tenant_id, projects=None):
        """初始化并返回用户有权限的项目"""

        projects = projects if projects is not None else get_user_projects(username, tenant_id)
    ...
    def list(self, request, *args, **kwargs):
        user_projects = get_user_projects(request.user.username, request.user.tenant_id)
        ...
        if not self.queryset.exists():
            self.queryset = self.get_default_projects(request.user.username, request.user.tenant_id, user_projects)
