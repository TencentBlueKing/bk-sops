...
def paginate_list_data(view_func):
    @wraps(view_func, assigned=available_attrs(view_func))
    def wrapper(request, *args, **kwargs):
        if request.method != "GET":
            return JsonResponse(
                {
                    "result": False,
                    "message": "invalid request method, need GET method.",
        ...
        try:
            ...
            if offset < 0 or limit < 0:
                return JsonResponse(
                    {
                        "result": False,
                        "message": "offset and limit must be greater or equal to 0.",
            ...
            result = json_response.get("result", True)
            if result is False:
                return response
            ...
            if not isinstance(data, list):
                return JsonResponse(
                    {
                        "result": False,
                        "message": "paginate object should be a list",
                        "code": err_code.VALIDATION_ERROR.code,
                    }
                )
            json_response["count"] = len(data)
            ...
            if limit == 0 and offset == 0:
                ...
                # 正常取值情况
                json_response["data"] = data[offset : offset + limit]
