...
from gcloud.utils.dates import format_datetime  # 导入format_datetime函数
...
def inject_timezone_for_time_fields(deserialized_instances, queryset, tz=None):
    """
    对序列化后的数据中的时间字段按指定时区(或默认本地时区)重新格式化，
    保持返回结构统一，不因是否传入 expected_timezone 而产生字段差异。

    :param deserialized_instances: 序列化后的字典列表（最终返回给前端，需要被修改的目标）
    :param queryset: 原始 ORM 实例列表，持有原始 datetime 对象，作为重新格式化的数据源
    :param tz: 目标时区，None 时使用系统默认本地时区
    """
    time_fields = ("plan_start_time", "create_time", "edit_time")
    for item, task in zip(deserialized_instances, queryset):
        for field in time_fields:
            item[field] = format_datetime(getattr(task, field), tz)
    return deserialized_instances
...
def get_clocked_task_list(request, project_id):
    ...
    # 统一对时间字段按时区格式化：用户传 expected_timezone 则用指定时区，
    # 否则使用系统默认本地时区，确保返回字段结构不受时区参数影响
    tz = getattr(request, "tz", None)
    deserialized_instances = inject_timezone_for_time_fields(deserialized_instances, paginated_queryset, tz)
    ...
    template_view_actions = get_flow_allowed_actions_for_user(
        ...
        [inst.template_id for inst in paginated_queryset],
