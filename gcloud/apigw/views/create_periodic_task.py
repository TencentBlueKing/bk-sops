...
def create_periodic_task(request, template_id, project_id):
    ...
    if PeriodicTask.objects.filter(project__id=project.id).count() >= periodic_task_limit:
        message = f"周期任务创建失败: 项目内的周期任务数不可超过: {periodic_task_limit} | create_periodic_task"
        logger.error(message)
        return {"result": False, "message": _(message), "code": err_code.INVALID_OPERATION.code}
    ...
    if template_source in NON_COMMON_TEMPLATE_TYPES:
        ...
        try:
            ...
            logger.error(f"任务创建失败: 任务关联的公共流程[ID: {template_id}]已不存在, 请检查流程是否存在 | create_periodic_task")
            result = {
                "result": False,
