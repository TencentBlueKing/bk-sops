...
class LabelManager(models.Manager):
    ...
    def get_project_label_with_default(self, project_id):
        return self.filter(Q(project_id=project_id) | (Q(is_default=True) & Q(project_id=-1))).order_by(
            Convert("name", "gbk")
        )

    def get_common_label_with_default(self):
        return self.filter(from_space_id__isnull=False, project_id__isnull=True).order_by(Convert("name", "gbk"))
...
class Label(models.Model):
    ...
    # 默认项目标签时project_id=-1
    project_id = models.IntegerField(_("项目 ID"), help_text="标签对应project id", null=True, blank=True)
    ...
    from_space_id = models.IntegerField(_("来源空间 ID"), help_text="创建标签的空间 ID", null=True, blank=True)
    ...
    class Meta:
        ...
        unique_together = ("project_id", "name", "from_space_id")
...
class TemplateLabelManager(models.Manager):
    def set_labels_for_template(self, template_id, label_ids, template_source=PROJECT):
        existing_labels = self.filter(template_id=template_id, template_source=template_source).values_list(
            "label_id", flat=True
        )
        add_labels = list(set(label_ids).difference(set(existing_labels)))
        add_relations = [
            TemplateLabelRelation(template_id=template_id, label_id=label_id, template_source=template_source)
    ...
    def fetch_labels_for_templates(self, template_ids, template_source=PROJECT):
        label_ids = (
            self.filter(template_id__in=template_ids, template_source=template_source)
            .distinct()
    ...
    def fetch_templates_labels(self, template_ids, label_fields=("name", "color"), template_source=PROJECT):
    ...
    def fetch_common_labels_for_templates(self, template_ids, label_fields=("name", "color"), template_source=PROJECT):
        label_ids = (
            self.filter(template_id__in=template_ids, template_source=template_source)
            .values_list("label_id", flat=True)
    ...
    def fetch_template_ids_using_labels(self, label_ids, template_source=PROJECT):
        template_ids = (
            self.filter(label_id__in=label_ids, template_source=template_source)
            .distinct()
            .values_list("template_id", flat=True)
        )
        return template_ids

    def fetch_template_ids_using_union_labels(self, label_ids, template_source=PROJECT):
        template_ids = (
            self.filter(label_id__in=label_ids, template_source=template_source)
            .values_list("template_id", flat=True)
    ...
    def fetch_label_template_ids(self, label_ids, template_source=PROJECT):
        relations = self.filter(label_id__in=label_ids, template_source=template_source)
        template_cls = (
            apps.get_model("template", "CommonTemplate")
            if template_source == COMMON
            else apps.get_model("tasktmpl3", "TaskTemplate")
        )
        exist_template_ids = set(
            template_cls.objects.filter(
                id__in=[relation.template_id for relation in relations], is_deleted=False
            ).values_list("id", flat=True)
        )

        label_template_ids = defaultdict(list)
        for relation in relations:
            if relation.template_id in exist_template_ids:
                label_template_ids[relation.label_id].append(relation.template_id)
        return label_template_ids

    def delete_relations_based_on_template(self, template_id, template_source=PROJECT):
        self.filter(template_id=template_id, template_source=template_source).delete()

    def delete_relations_based_on_label(self, label_id, template_source=PROJECT):
        self.filter(label_id=label_id, template_source=template_source).delete()
...
class TemplateLabelRelation(models.Model):
    ...
    class Meta:
        ...
        unique_together = ("template_id", "label_id", "template_source")
