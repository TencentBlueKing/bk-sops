...
def form(request, project_id):
    ...
    try:
        ...
        return JsonResponse({"result": False, "message": "there is no template with id(%s)" % template_id})
    ctx = {
...
def collect(request, project_id):
    ...
    if template_id:
        ...
        try:
            ...
            return JsonResponse({"result": False, "message": "flow[id=%s] does not exist" % template_id})
    ...
    if template_list:
        ...
        try:
            templates = TaskTemplate.objects.filter(pk__in=template_list, project_id=project_id, is_deleted=False)
            perms_tuples = [(task_template_resource, [task_template_resource.actions.view.id], t) for t in templates]
            batch_verify_or_raise_auth_failed(
                principal_type="user", principal_id=request.user.username, perms_tuples=perms_tuples
            )
...
def export_templates(request, project_id):
    ...
    try:
        ...
        return JsonResponse({"result": False, "message": "Invalid template id list"})
...
def check_before_import(request, project_id):
    ...
    return JsonResponse({"result": True, "data": check_info, "permission": permissions})
...
def import_preset_template_and_replace_job_id(request, project_id):
    ...
    if job_id_map_json is None:
        return JsonResponse({"result": False, "message": "job_id_map can not be None"})

    # replace str to int
    try:
        job_id_map = json.loads(job_id_map_json)
    except Exception:
        return JsonResponse({"result": False, "message": "job_id_map is not a valid json string"})
    ...
    try:
        ...
        return JsonResponse(
            {"result": False, "message": "invalid flow data or error occur, please contact administrator"}
        )
...
def replace_all_templates_tree_node_id(request):
    ...
    return JsonResponse({"result": True, "data": {"total": total, "success": success}})
...
def job_id_map_convert(origin_id_maps):
    ...
    for id_map in origin_id_maps:
        _map = {"id": int(id_map["new_job_id"]), VAR_ID_MAP: {}}
...
def get_templates_with_expired_subprocess(request, project_id):
    return JsonResponse(
        {"result": True, "data": TaskTemplate.objects.get_templates_with_expired_subprocess(project_id)}
    )
