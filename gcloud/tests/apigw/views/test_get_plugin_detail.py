...
class GetPluginDetailAPITest(APITest):
    def url(self):
        return "/apigw/get_plugin_detail/{project_id}/?code={code}"
    ...
    def test_get_component_detail(self):
        ...
        mock_get = MagicMock()
        mock_get.get = MagicMock(return_value=comp_model)
        mock_filter = MagicMock(return_value=mock_get)
        with mock.patch(APIGW_GET_PLUGIN_DETAIL_COMPONENT_MODEL_EXCLUDE, mock_filter):
            with mock.patch(
