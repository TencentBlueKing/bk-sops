...
class ModifyTemplateNotifyTmpAPITest(APITest):
    ...
    def setUp(self):
        ...
        # Mock CMSI APIGateway 客户端获取通知类型（覆盖 postman 用例中出现的全部类型）
        self.mock_cmsi_client = MagicMock()
        self.mock_cmsi_client.api.v1_channels_list.return_value = {
            "data": [
                {"type": "weixin"},
                {"type": "mail"},
                {"type": "sms"},
        ...
        self.mock_get_client_by_username = MagicMock(return_value=self.mock_cmsi_client)
