...
from gcloud.plugin_gateway.tasks import sweep_expired_plugin_gateway_runs
...
class PluginGatewayExecutionServiceTestCase(TestCase):
    def setUp(self):
        ...
        self.get_plugin_reference_patcher = patch(
            "gcloud.plugin_gateway.services.execution.PluginGatewayCatalogService.get_plugin_reference",
            side_effect=self._get_plugin_reference,
        )
    ...
    def test_create_run_persists_context_and_created_state(self):
        payload = dict(self.payload)
        payload["context"] = {"scope_type": "biz", "scope_value": "2", "operator": "zhangsan"}

        run, created = PluginGatewayExecutionService.create_run("bkflow-app", payload)
        ...
        self.assertIsNotNone(run.execution_expire_at)

    def test_create_run_rejects_do_not_open_plugin(self):
        self.source_config.do_not_open_list = ["plugin_job_execute"]
    ...
    def test_sweep_marks_expired_running_failed(self, mock_callback_run):
        run = self._create_run(
    ...
    @patch("gcloud.plugin_gateway.services.execution.PluginGatewayCallbackService.callback_run")
    def test_cancel_already_terminal_is_idempotent(self, mock_callback_run):
        run = self._create_run(run_status=PluginGatewayRun.Status.SUCCEEDED)

        result = PluginGatewayExecutionService.cancel_run(run.open_plugin_run_id, "bkflow-app")

        self.assertEqual(result.run_status, PluginGatewayRun.Status.SUCCEEDED)
        mock_callback_run.assert_not_called()

    @patch("gcloud.plugin_gateway.services.execution.PluginGatewayCallbackService.callback_run")
    def test_cancel_underlying_failure_still_marks_cancelled(self, mock_callback_run):
        run = self._create_run()

        with patch.object(
            PluginGatewayExecutionService,
            "_cancel_underlying_best_effort",
