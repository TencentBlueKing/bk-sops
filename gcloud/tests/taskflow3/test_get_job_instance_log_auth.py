...
def _build_request(username="tester", job_instance_id="100"):
    return SimpleNamespace(
        method="GET",
        user=SimpleNamespace(username=username, tenant_id="system"),
...
class GetJobInstanceLogAuthTestCase(TestCase):
    ...
    @mock.patch("gcloud.taskflow3.apis.django.api.get_client_by_username")
    ...
    def test_iam_called_with_internal_project_id_not_bk_biz_id(
        ...
        client.api.get_job_instance_ip_log.return_value = {"result": True, "data": "log"}
        mocked_get_client_by_username.return_value = client
        request = _build_request()

        response = api.get_job_instance_log(request, biz_cc_id="123")

        mocked_project_get.assert_called_once_with(bk_biz_id="123", tenant_id="system")
        mocked_resources_for_project.assert_called_once_with(42, "system")
    ...
    def test_iam_denied_short_circuits_before_calling_job(
        ...
        mocked_get_client_by_username.assert_not_called()
