...
class MockCompoentData(MagicMock):
    def filter(self, *args, **kwargs):
        return [
            {"component_code": "code1", "version": "1.0", "is_remote": False, "elapsed_time": 10},
            {"component_code": "code2", "version": "1.0", "is_remote": True, "elapsed_time": 1},
        ]
...
TEST_GROUPS = [
    {"code": "code1-1.0", "name": "组-name1-1.0", "value": 10},
        ...
        with mock.patch(MOCK_COMP_TOTAL, MagicMock(return_value=TEST_TOTAL)) as mock_total:
            with mock.patch(MOCK_COMP_VALUES_LIST, MagicMock(return_value=TEST_COMP_NAME)) as mock_dict:
                with mock.patch(
                    ...
                    total, groups = TaskFlowInstance.objects.group_by_atom_avg_execute_time(MockTaskFlow())
                    mock_total.assert_called_once_with()
                    mock_dict.assert_called_once_with("code", "name")
                    mock_comp_data.assert_called_once_with("component_code", "version", "elapsed_time", "is_remote")
