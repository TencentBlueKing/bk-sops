...
class CommonProjectResource(GCloudModelResource):
    ...
    class Meta(GCloudModelResource.Meta):
        ...
        auth_resource = project_resource
        authorization = BkSaaSReadOnlyAuthorization(
            auth_resource=auth_resource, read_action_id="view", resource_f="project"
        )
        allowed_methods = ["get"]
        filtering = {
            "id": ALL,
    ...
    def get_default_projects(empty_query, username):
        ...
        if not authorized_projects["result"]:
            logger.error("Get authorized projects error: {error}".format(error=authorized_projects["message"]))
            project_ids = []
        else:
