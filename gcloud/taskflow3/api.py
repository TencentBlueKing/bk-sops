...
def context(request):
    ...
    @return:
    """
    ctx = {"result": True, "data": TaskContext.flat_details()}
...
def task_clone(request, project_id):
    ...
    ctx = {"result": True, "data": {"new_instance_id": new_task_id}}
    return JsonResponse(ctx)
...
def task_modify_inputs(request, project_id):
    ...
    task = TaskFlowInstance.objects.get(pk=task_id, project_id=project_id)
    if task.is_started:
        ctx = {"result": False, "message": "task is started"}
    elif task.is_finished:
        ctx = {"result": False, "message": "task is finished"}
    else:
        try:
            constants = json.loads(request.POST.get("constants"))
        except Exception:
            logger.error("load taskflow modify constants failed: %s" % traceback.format_exc())
            ctx = {"result": False, "message": "invalid constants format"}
        else:
...
def get_taskflow_root_context(request, taskflow_id):
    try:
        ...
        return JsonResponse({"result": False, "message": "taskflow with id[%s] does not exist." % taskflow_id})
