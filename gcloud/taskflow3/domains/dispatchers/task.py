...
class TaskCommandDispatcher(EngineCommandDispatcher):
    ...
    def start_v2(self, executor: str) -> dict:
        ...
        if not update_success:
            logger.error("任务操作失败: 已启动的任务不可再次启动 | start_v2")
            return {
                "result": False,
        ...
        try:
            ...
            logger.error(f"任务启动失败: 引擎启动失败, {e}. 请重试. 如持续失败可联系管理员处理 | start_v2")
            return {
                "result": False,
                "message": _(f"任务启动失败: 引擎启动失败, {e}. 请重试. 如持续失败可联系管理员处理 | start_v2"),
                "code": err_code.UNKNOWN_ERROR.code,
            }
    ...
    def set_task_constants(
        ...
        if pre_render_constants or hide_constants or component_outputs:
            logger.error(
                f"任务参数设置失败: 常量 {pre_render_constants}、输出参数 {hide_constants}、隐藏变量 {component_outputs} "
                f"不可修改值, 请检查变量配置 | set_task_constants"
            )
            return {
                "result": False,
                "message": _(
        ...
        try:
            ...
            logger.error("任务参数设置失败: 非法的任务参数, 请修改后重试 | set_task_constants")
            return {
                "result": False,
        ...
        with transaction.atomic():
            if task_is_started:
                ...
                if not update_res.result:
                    logger.error(
                        f"任务参数设置失败: 更新引擎上下文发生异常: {update_res.exc_trace}. 请重试, 如持续失败可联系管理员处理 | set_task_constants"
                    )
                    return {
                        "result": False,
                        "data": "",
    ...
    def get_task_status_v1(self, subprocess_id: Optional[str], with_ex_data: bool) -> dict:
        ...
        if not subprocess_id:
            try:
                ...
                logger.error("任务数据请求失败: 请重试, 如持续失败可联系管理员处理 | get_task_status_v1")
                return {
                    "result": False,
            ...
            try:
                ...
                logger.exception(f"任务数据请求失败: 请重试, subprocess_id = {subprocess_id}, 如持续失败可联系管理员处理 | get_task_status_v1")
                return {
                    "result": False,
                    "message": _(f"任务数据请求失败: 请重试, subprocess_id = {subprocess_id}, 如持续失败可联系管理员处理 | get_task_status_v1"),
                    "data": {},
                    "code": err_code.UNKNOWN_ERROR.code,
                }
