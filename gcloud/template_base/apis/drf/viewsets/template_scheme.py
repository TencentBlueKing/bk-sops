...
class TemplateSchemeViewSet(ApiMixin, viewsets.ModelViewSet):
    ...
    def get_pipeline_template_id(template_id, *args, **kwargs):
        ...
        if not template:
            ...
            message = _(
                "获取流程失败: 流程ID为[{template_id}]]的[{template_type}]类型流程不存在".format(
    ...
    def batch_operate(self, request, *args, **kwargs):
        ...
        if remove_scheme_ids_set & quote_scheme_ids_set:
            message = _("执行方案删除失败: 待删除的[执行方案]已被引用[{}], 请处理后重试".format(quote_scheme_ids_set))
        ...
        try:
            ...
            message = _("执行方案批量操作失败: 流程ID: {}, 失败原因: {}".format(template_id, str(e)))
            logger.error(message)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if scheme_quote_num != 0:
            message = _("执行方案删除失败: 待删除的[执行方案]已被引用[{}], 请处理后重试".format(scheme_quote_num))
            return Response({"detail": ErrorDetail(message, err_code.REQUEST_PARAM_INVALID.code)}, exception=True)
