...
def common_template_creat_related_actions_handler(sender, instance, created, **kwargs):
    application = common_flow_params(instance, IAMMeta.TASK_RESOURCE, ancestors=True)

    ok, message = iam.grant_resource_creator_actions(application, bk_username=instance.creator)
    if not ok:
        logging.error("Failed to register resource for 'TASK_RESOURCE',resources info:%s, response message:%s" % (
            application, message))
