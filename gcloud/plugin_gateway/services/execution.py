...
class PluginGatewayExecutionService:
    ...
    def create_run(cls, caller_app_code, payload):
        ...
        defaults = {
            ...
            "execution_expire_at": timezone.now() + timedelta(seconds=source_config.execution_timeout_seconds),
        }
    ...
    def cancel_run(cls, open_plugin_run_id, caller_app_code):
        ...
        try:
            cls._cancel_underlying_best_effort(run)
        except Exception:
    ...
    @staticmethod
    def _cancel_underlying_best_effort(run):
        logger.info("[plugin_gateway] best-effort cancel run=%s", run.open_plugin_run_id)
    ...
    @staticmethod
    def _validate_do_not_open_plugin(source_config, plugin_id):
        if plugin_id in (source_config.do_not_open_list or []):
            raise PluginGatewayPluginNotEnabledError(
                "plugin({}) is in do_not_open_list for source({})".format(plugin_id, source_config.source_key)
            )
