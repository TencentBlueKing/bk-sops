# -*- coding: utf-8 -*-
"""
导出指定租户的流程 Mako 表达式清单，并按 bamboo-engine PR #269 安全策略打标。

使用方法:
    python manage.py export_mako_expression_inventory --tenant-id TENANT --output /tmp/mako_exprs.csv
    python manage.py export_mako_expression_inventory --tenant-id TENANT --output /tmp/mako_exprs.csv --hits-only
    python manage.py export_mako_expression_inventory --tenant-id TENANT --output /tmp/mako_exprs.csv --project-id 23
    python manage.py export_mako_expression_inventory --tenant-id TENANT --output /tmp/mako_exprs.csv \\
        --bk-biz-id 1001,1002
    python manage.py export_mako_expression_inventory --tenant-id TENANT --output /tmp/mako_exprs.csv \\
        --base-url https://apps.example.com/o/bk_sops --skip-subprocess-exec

必须显式指定一个租户；项目、业务筛选均在该租户内生效。
默认输出该租户所有流程里的全部 ${...}，可用 --hits-only 或 --project-id 缩小范围。
...
class Command(BaseCommand):
    help = "导出指定租户的流程 Mako 表达式清单，并按 PR #269 安全策略打标"

    def add_arguments(self, parser):
        parser.add_argument("--output", type=str, required=True, help="输出 CSV 路径")
        parser.add_argument("--tenant-id", type=str, required=True, help="租户 ID；一次只导出一个租户")
        parser.add_argument(
            "--source",
            type=str,
            default="all",
    ...
    def handle(self, *args, **options):
        self.tenant_id = (options.get("tenant_id") or "").strip()
        if not self.tenant_id:
            raise CommandError("必须通过 --tenant-id 指定一个非空租户 ID")
        ...
        if bk_biz_ids:
            matched = list(
                Project.objects.filter(tenant_id=self.tenant_id, bk_biz_id__in=bk_biz_ids).values_list("id", flat=True)
            )
            if project_ids:
        ...
        self.stdout.write("租户: {}".format(self.tenant_id))
    ...
    def _template_queryset(self, model, source, project_ids, include_deleted):
        if source == PROJECT:
            qs = model.objects.filter(project__tenant_id=self.tenant_id)
        else:
            qs = model.objects.filter(tenant_id=self.tenant_id)
    ...
    def _load_project_map(self, project_ids):
        qs = Project.objects.filter(tenant_id=self.tenant_id)
        if project_ids:
    ...
    def _load_business_names(self, project_map):
        ...
        if not biz_ids:
            return {}
        return dict(
            Business.objects.filter(tenant_id=self.tenant_id, cc_id__in=biz_ids).values_list("cc_id", "cc_name")
        )
        ...
        for id_chunk in chunked(template_ids, chunk_size):
            stats = (
                TaskFlowInstance.objects.filter(
                    project__tenant_id=self.tenant_id,
                    is_deleted=False,
    ...
    def _map_parent_templates(self, parent_pipeline_ids, chunk_size):
        ...
        for id_chunk in chunked(parent_pipeline_ids, chunk_size):
            for row in TaskTemplate.objects.filter(
                project__tenant_id=self.tenant_id, pipeline_template_id__in=id_chunk, is_deleted=False
            ).values("id", "pipeline_template_id"):
            ...
            for row in CommonTemplate.objects.filter(
                tenant_id=self.tenant_id, pipeline_template_id__in=id_chunk, is_deleted=False
            ).values("id", "pipeline_template_id"):
    ...
    def _verify_parent_latest(self, parent_latest, pipeline_to_parent_keys, target_to_ancestors, chunk_size):
        ...
        for ancestor, info in parent_latest.items():
            ...
            if not task_id:
                continue
            row = (
                TaskFlowInstance.objects.filter(id=task_id, project__tenant_id=self.tenant_id)
                .values("pipeline_instance__execution_snapshot_id")
                .first()
            )
            if not row:
                continue
            ...
            for id_chunk in chunked(ids, chunk_size):
                rows = (
                    ClockedTask.objects.filter(
                        # ClockedTask stores an integer project_id, not a foreign key.
                        project_id__in=Project.objects.filter(tenant_id=self.tenant_id).values("id"),
                        template_source=source,
                        template_id__in=id_chunk,
