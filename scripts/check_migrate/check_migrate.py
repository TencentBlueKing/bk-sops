...
def handle_rename_model(file_path, library):
    ...
    for _, value in rename_content.items():
        ...
        if suggested_naming:
            err_list.append(
                'Field ({}) in Model ({}) might be replaced by ({})'.format(value['new_name'],
                                                                            value['model_name'],
                                                                            suggested_naming))
    return err_list
...
def handle_add_alter_model(file_path, library):
    ...
    for _, value in add_alter_content.items():
        ...
        if suggested_naming:
            err_list.append(
                'Field ({}) in Model ({}) might be replaced by ({})'.format(value['name'],
                                                                            value['model_name'],
                                                                            suggested_naming))
    return err_list
...
def handle_create_model(file_path, library):
    ...
    for model_name, fields in fields_dict.items():
        for field in fields:
            ...
            if suggested_naming:
                err_list.append('Field ({}) in Model ({}) might be replaced by ({})'.format(field,
                                                                                            model_name,
                                                                                            suggested_naming))
    return err_list
