const cn = {
    ...
    '自动隐藏': '自动隐藏',
    '满足触发条件的变量，在任务执行填参页面将自动隐藏。可用来实现特定条件下忽略必填参数': '满足触发条件的变量，在任务执行填参页面将自动隐藏。可用来实现特定条件下忽略必填参数',
    '启动失败': '启动失败',
    '节点ID': '节点ID',
    '表格设置': '表格设置',
    '字段显示设置': '字段显示设置',
    ...
    '变量': '变量',
    '以后再说': '以后再说',
    '全局变量【 x 】的引用数已为 0。如果不再使用，可立即删除变量; 也可以稍后再全局变量面板中删除': '全局变量【 {key} 】的引用数已为 0。如果不再使用，可立即删除变量; 也可以稍后再全局变量面板中删除',
    '取消变量豁免': '取消变量豁免',
    '变量豁免': '变量豁免',
    '取消变量引用': '取消变量引用',
    '变量引用': '变量引用',
    '变量引用和豁免使用指引': '变量引用和豁免使用指引',
    '变量引用：使用参数的配置创建全局变量并引用，或不创建直接引用同类型变量': '变量引用：使用参数的配置创建全局变量并引用，或不创建直接引用同类型变量',
    '变量豁免：开启后忽略参数中的全局变量，视为普通字符串': '变量豁免：开启后忽略参数中的全局变量，视为普通字符串',
    ...
    '任务名': '任务名',
    'ID/任务名/创建人/执行人/任务类型/状态': 'ID/任务名/创建人/执行人/任务类型/状态',
