const cn = {
    ...
    '设置为常量表示在任务执行最开始完成值的渲染，后续执行过程中值保存不变': '设置为常量表示在任务执行最开始完成值的渲染，后续执行过程中值保存不变',
    '顶层流程（n）': '顶层流程（{n}）',
    '子流程（n）': '子流程（{n}）',
    ...
    '设置了模板预渲染的变量，不支持中途修改参数值': '设置了模板预渲染的变量，不支持中途修改参数值',
    '分支类型': '分支类型',
    '自定义分支': '自定义分支',
    '默认分支': '默认分支',
    ...
    '支持DAT类型文件，文件小于2M': '支持DAT类型文件，文件小于2M',
    '上传失败，DAT类型文件最大为2M': '上传失败，DAT类型文件最大为2M',
    '上传失败，YAML类型文件最大为2M': '上传失败，YAML类型文件最大为2M',
    '退出登录': '退出登录',
    '执行时显示': '执行时显示',
