const cn = {
    ...
    '模板预渲染为是的变量，会在开始节点执行时就完成渲染，引用了节点输出的变量开启预渲染后会渲染失败': '模板预渲染为是的变量，会在开始节点执行时就完成渲染，引用了节点输出的变量开启预渲染后会渲染失败',
    '顶层流程（n）': '顶层流程（{n}）',
    '子流程（n）': '子流程（{n}）',
    '是否导入子流程（仅适用于同环境导入）': '是否导入子流程（仅适用于同环境导入）',
    '不覆盖，新建流程': '不覆盖，新建流程',
    '覆盖已有流程': '覆盖已有流程',
    '不导入，使用已有流程': '不导入，使用已有流程',
    '请选择需要使用的流程': '请选择需要使用的流程',
    '公共流程管理': '公共流程管理',
