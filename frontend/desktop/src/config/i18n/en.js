const en = {
    ...
    '确定保存流程并去设置执行方案？': 'Are you sure you want to save the flow and go to  step groups?',
    '确定保存克隆流程并去设置执行方案？': 'Are you sure you want to save clone flow and go to step groups?',
    ...
    '执行代理人设置': 'Representative Settings',
    '执行代理人': 'Representative',
    ...
    '执行代理人在任务开始执行时确定，未执行任务不展示': 'The representative is determined when the task starts to execute, and the unexecuted task will not be displayed',
    ...
    '确认离开？': 'Are you sure you want to leave?',
    ...
    '任务名': 'TaskName',
    'ID/任务名/创建人/执行人/状态/创建方式/执行代理人': 'ID/TaskName/Created by/Executed By/Status/Create Method/Representative',
    ...
    '拥有流程管理权限的人员，通过设置“执行代理人”功能，将流程任务的执行操作交由第三方人员（如：外包、外聘人员），帮助流程管理人员从繁重的执行工作中解放。': 'Person who owe the flow management permission can assign the execution of tasks to the helper team (such as outsourced, external personnel) by setting the "Representative" , which helps the flow managers to be freed from heavy work.',
    ...
    '计划任务删除成功': 'One-Time Scheduled task deleted successfully',
    '周期任务删除成功': 'Scheduled task deleted successfully',
    ...
    '离开': 'Leave',
    '当前任务为旧版周期任务，无法判断创建周期任务后流程数据是否变更。可前往编辑任务，完成一次更新即升级到新版本，获得新版本的提示更新能力。': 'The current task is an old version of the  scheduled task and cannot determine if the flow has changed after creating the scheduled task. You can go to edit the task and complete one update to get the prompt update ability of the new version.',
    '确定强制终止当前节点？': 'Are you sure you want to forced terminate of the current node?',
    ...
    '重新执行': 'Redo',
    '复⽤参数值并使⽤流程最新数据重新执行': 'Redo using the latest version of the flow and reuse the parameter values of current task',
    '请选择流程进行节点配置': 'Please select a process for node configuration',
    ...
    '是否为选择时间范围': 'Whether to choose time range',
    '搜索分组': 'Search group',
    '列表': 'List',
    ...
    '从{0}分开始到{1}分的每{2}分钟': 'Every {2} minutes from {0} minutes to {1} minutes',
    '每小时': 'Every hour',
    '{0}到{1}': '{0} to {1}',
