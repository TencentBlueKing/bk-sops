const en = {
    ...
    '还原': 'Restore',
    '鼠标左键单击': 'Left-Click',
    '连续选中（或取消）节点': 'Select(Un-Select) nodes',
    ...
    '已选流程模板没有查看权限，请取消选择或申请权限': 'No view permission, un-select or apply for permission',
    '已选流程模板没有编辑权限，请取消选择或申请权限': 'No edit permission, un-select or apply for permission',
    ...
    '方式引用': 'with ',
    '可以在模板中通过': 'It can be used in the flow ',
    '项目级别的变量建立后': 'When the project variables are created',
    '值': 'Value',
    ...
    '请先保存方案再执行其他操作': 'Save the plan before performing other operations',
    '新增': 'Add',
    '预览节点': 'Preview node',
    ...
    '确定保存流程并去设置执行方案？': 'Save the flow and go to  step-groups?',
    '确定保存克隆流程并去设置执行方案？': 'Save the flow and go to step-groups?',
    ...
    '如果轻应用选择了执行方案，更新模板后需要同步更新执行方案。': 'If the mini-app Selects the step-group, it needs to update the step-group synchronously after updating the flow.',
    '应用LOGO': 'LOGO',
    '点击更换': 'Change',
    ...
    '修改周期任务失败，请联系管理员': 'Modify scheduled task failed, contact admin ',
    ...
    '选中状态': 'Status',
    '参考格式：步骤：节点名 标识位(0：不选择；1：选择；2：非可选节点)，并以换行符分隔。': 'reference format：Stage：Node Status(0:un-select; 1:select; 2:non-selectable node), separated by a new line.',
    '方案中的（': 'The node (',
    ...
    '启动/暂停': 'Start/Pause',
    '已选节点': 'Selected node',
    '当前任务为旧数据，仅记录已选节点，强制更新后可选执行方案并获得提示更新能力': 'This task is the old data, only the selected node is recorded, after the forced update, the step-group can be selected and the prompt update ability can be obtained',
    '已排除节点': 'Node excluded',
    '当前任务为旧数据，仅记录已排除节点，可重选执行方案获得跟随执行方案更新能力': 'This task is the old data, only the excluded nodes are recorded, and the step-group can be reselected to obtain the ability to follow the step-group update',
    '无': 'None',
    '请选择执行方案': 'Select an implementation plan',
    ...
    '离开': 'Leave',
    '当前任务为旧版周期任务，无法判断创建周期任务后流程数据是否变更。可前往编辑任务，完成一次更新即升级到新版本，获得新版本的提示更新能力。': 'This task is an old version of the  scheduled task and cannot determine if the flow has changed after creating the scheduled task. You can go to edit the task and complete one update to get the prompt update ability of the new version.',
