const en = {
    ...
    '重复添加连线': 'Duplicated connection',
    '已达到': ' has reached ',
    '最大输出连线条数': ' Output connections count exceeded',
    '最大输入连线条数': ' Input connections count exceeded',
    ...
    'API网关': 'API Gateway',
    '重置': 'Reset',
    '模板执行次数TOPn': 'TOP{n} flow execution times',
    ...
    '旧版': 'Old',
    '返回编辑': 'Back To Edit',
    '周期任务根据创建时的流程和执行方案数据生成快照保存，流程变更后不影响周期任务，可手动更新到使用流程最新数据。': 'Scheduled tasks generate snapshots and save according to the flow and node-group data at the time of creation. After the flow is changed, it does not affect the scheduled tasks, and can be manually updated to use the latest version of the flow.',
    ...
    '计划任务在执行时获取最新的流程和执行方案数据创建任务，流程和方案变更将影响未执行的计划任务，如增加参数可能导致计划任务启动失败。': 'When a one-time scheduled task is executed, it get the latest version from the flow and node-group  to create a task. Changes to the flow and plan will affect the unexecuted one-time scheduled task. For example, adding parameters may cause the one-time scheduled task to fail to start.',
    ...
    '不再支持周期任务使用公共流程，请使用项目流程': 'The scheduled tasks are no longer supported to use common flow, please use project flow',
    '常量': 'Constant',
    '必填': 'Required',
    '非必填': 'Not required',
