...
def build_need_permission(auth_resource, action_id, instance=None, scope_id=None):
    ...
    if instance is not None:
        ...
        resource.update({"resource_id": resource_id, "resource_name": auth_resource.resource_name(instance)})
...
def search_instance_authorized_actions(
    username, resource_type, auth_resource, instance_id, instance, action_ids=None, scope_id=None
):
    if action_ids is None:
        action_ids = list(auth_resource.actions_map.keys())
    ...
    if not verify_result["result"]:
        logger.error(
            "Search authorized actions of Resource[{resource}] return error: {error}".format(
                resource=auth_resource.name, error=verify_result["message"]
            )
        )
        return []
