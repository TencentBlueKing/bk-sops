...
class BkSaaSReadOnlyAuthorization(ReadOnlyAuthorization):
    ...
    def build_auth_failed_response(self, action_ids, instance, auth_resource=None, scope_id=None):
        ...
        if isinstance(action_ids, list):
            permission = [
                build_need_permission(
                    auth_resource=auth_resource, action_id=action_id, instance=instance, scope_id=scope_id
                )
            ...
            permission = [
                build_need_permission(
                    auth_resource=auth_resource, action_id=action_ids, instance=instance, scope_id=scope_id
                )
            ]
        return ImmediateHttpResponse(HttpResponseAuthFailed(permission))

    def authorized_list(self, username, action_id, scope_id=None):
        authorized_result = self.auth_resource.backend.search_authorized_resources(
        ...
        if not authorized_result["result"]:
            logger.error(
                "Search authorized resources of Resource[{resource}] return error: {error}".format(
                    resource=self.auth_resource.name, error=authorized_result["message"]
                )
            )
            return []
    ...
    def is_allow(self, bundle, action_ids, auth_resource=None, instance=None):
        auth_resource = auth_resource or self.auth_resource
        instance = instance or bundle.obj
        username = bundle.request.user.username

        authorized_result = auth_resource.verify_perms(
            principal_type=self.principal_type, principal_id=username, action_ids=action_ids, instance=instance
        )
        if not authorized_result["result"] or not authorized_result["data"][0]["is_pass"]:
            if not authorized_result["result"]:
                logger.error(
                    "Verify perms of Resource[{resource}] return error: {error}".format(
                        resource=auth_resource.name, error=authorized_result["message"]
                    )
                )
            raise self.build_auth_failed_response(
                action_ids, instance, auth_resource=auth_resource, scope_id=self.scope_id_for_bundle(bundle)
            )
    ...
    def read_detail(self, object_list, bundle):
        return self.is_allow(bundle, [self.read_action_id])
...
class BkSaaSAuthorization(BkSaaSReadOnlyAuthorization):
    ...
    def create_detail(self, object_list, bundle):
        ...
        return self.is_allow(bundle, action_ids, auth_resource, instance)
    ...
    def update_detail(self, object_list, bundle):
        return self.is_allow(bundle, [self.update_action_id])
    ...
    def delete_detail(self, object_list, bundle):
        return self.is_allow(bundle, [self.delete_action_id])
