...
- 优化项
  - 流程模板兼容数据中 can_retry、isSkipped 字段不存在的情况
- bugfix
  - 修复任务节点失败后执行记录表头文案未显示问题
  - 修复TagInput默认被禁用的问题
...
- bugfix
  - 修复CDMB中主机云区域为空时，IP选择器接口报500的问题

## 3.4.17

- 新功能
  - 插件开发上下文增加获取项目id方法
  - 左侧工具栏“标准插件节点”点击后展开，再次点击收起
- 优化项
  - 模板编辑页面刷新或关闭页面增加二次确认提示
  - 节点框选复制后粘贴位置优化
- bugfix
  - 修复执行作业输出参数勾选后，编辑表格勾选取消问题
  - 修复 iframe 中平台上弹窗登录不显示的问题
  - 修复删除全局变量报错的问题
...
- 优化项
  - pipeline_tree 数据校验错误提示信息优化
  - 前端请求分块静态资源带版本号
- bugfix
  - 修复子流程更新参数值保留逻辑错误的问题
  - 修复添加子流程后配置项未清空的问题
  - 修复版本升级到V3.4.13后之前的周期任务无法执行的问题
  - 修复切换变量配置面板关闭的问题
  - 修复编辑周期任务参数值未回显的问题
  - 修复运营数据任务、标准插件表格倒序排列参数不正确的问题
  - 修复运营数据-任务统计中排序后导致页数丢失的问题
  - 修复【JOB-执行作业】插件全局参数引用整数类型变量解析失败的问题
  - 修复【CMDB-创建集群】插件集群容量参数未使用整数的问题
  - 修复【NODEMAN-安装】插件验证方式为 key 时加密出错的问题
  - 修复权限中心异常时后台返回 500 的问题
...
- 优化项
  - 停用项目隐藏「进入」「编辑」按钮
- bug fix
  - 修复节点编辑切换选择忽略错误，切换节点后手动处理选项被禁用的问题
  - 修复画布点击自动编排，再添加节点后，点击画布其他节点后新增节点会消失的问题
  - 修复任务创建页面调用自动排版接口参数错误的问题
  - 修复公共流程新建的任务，在任务执行页面跳转到公共流程模板列表链接错误的问题
  - 修复流程模板全局变量勾选到输出的切换开关显示的问题
  - 修复 admin 下公共流程新建跳转到template新建的问题
  - 修复项目流程导出文件能够在公共流程内覆盖的问题
  - 修复【JOB-分发本地文件】插件上传文件未保存的问题
  - 修复公共流程模板缓存异常的问题
  - 修复公共流程编辑子流程节点，点击子流程跳转链接错误的问题
