...
对应 `bamboo-pipeline==4.0.5`，其包依赖已固定 `bamboo-engine==3.0.6`，无需另外覆盖 engine 版本。

以上依赖已发布到 PyPI，并包含下述通过 `RenderInfrastructureError` 显式报告基础设施故障的行为。部署时需同时更新应用依赖和本次 bk-sops `SystemObject` 源码修复，以完整接入故障处理和系统上下文传输修复。以下默认值与主分支一致；HB / 多租户相对原分支的变化见下一节。

## HB / 多租户分支升级

本次补齐 `release_humming_bird` 和 `dev_multi_tenant` 在依赖升级后缺失的应用侧接入，行为与标准运维主分支一致。**导入表由原来的内置默认模块改为仅使用环境变量，未配置时为空**；白名单和隔离变量也从本次起实际绑定到引擎。不要只更新代码而遗漏部署变量。

若沿用已确定的生产兼容配置，在对应模块的部署配置中设置：
...
如已有其他业务导入模块，需合并到导入表中。将配置同步到 Web、实际承接任务的 Worker 及使用表达式渲染的其他模块，并随版本重启；环境变量按模块生效。该组合保留无条件安全检查，允许部分上下文兼容回退，但不回退启动、超时等基础设施故障。`NO_NETWORK=0` 表示不要求网络命名空间隔离。配置默认值与下表一致，这段部署示例不会自动改变默认值。
...
- 显式设置 `inprocess` 可切回进程内渲染。此时子进程池、超时、资源及网络隔离参数不参与渲染。
...
| 环境变量 | 默认值 | 引擎配置及作用 |
| --- | --- | --- |
| `BKAPP_MAKO_RENDER_BACKEND` | `subprocess` | `MAKO_RENDER_BACKEND`；默认使用隔离后端，显式 `inprocess` 可切回进程内 |
...
布尔项接受 `0/1` 或 `false/true`；后端名或布尔值拼写错误会使配置加载失败。数值项使用正数。采用上面的进程内兼容方案时需显式设置该后端；采用默认子进程方案时需核实隔离参数和容器权限。屏蔽词和 `_system` / `_loop` 白名单保留代码默认值，无需另配环境变量。
...
盘点命令由管理员按需执行，不会随发布自动扫描。它导出表达式、命中规则及流程链接，并可附带执行统计；只读数据库，不执行业务表达式。

```bash
python manage.py export_mako_expression_inventory --tenant-id TENANT_ID --output /tmp/mako_inventory.csv --hits-only
```

多租户分支必须显式指定 `--tenant-id`，所有模板、业务、执行记录和定时任务查询均限制在该租户内，CSV 附带租户 ID。
