...
| template_schemes_id | list/string/integer | NO | execution scheme ID list or single execution scheme ID. Supports string IDs returned by get_template_schemes and integer TemplateScheme IDs. It cannot be used together with pipeline_tree, non-empty exclude_task_nodes_id, or non-empty execute_task_nodes_id |
