...
Poll the status of a plugin gateway run. This endpoint is used by the `uniform_api v4.0.0` polling contract.
...
`status` can be `CREATED`, `RUNNING`, `WAITING_CALLBACK`, `SUCCEEDED`, `FAILED`, or `CANCELLED`. `RUNNING` is the value exposed by `polling.running_tag` in plugin detail.
