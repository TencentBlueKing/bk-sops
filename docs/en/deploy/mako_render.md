...
Use `bamboo-pipeline==4.0.5`, which pins `bamboo-engine==3.0.6`. Do not override the engine dependency separately.

These dependencies are available on PyPI and include the explicit infrastructure-failure behavior through `RenderInfrastructureError` described below. Deploy the dependency update together with this bk-sops `SystemObject` source fix to apply both failure handling and system-context transport fixes. The defaults match master; branch-specific changes are described in the next section.

## HB and multi-tenant branch upgrade

This backport completes the application integration missing from the dependency upgrades on `release_humming_bird` and `dev_multi_tenant`, matching the standard-operations master branch. **The import table changes from built-in defaults to environment-only configuration and is empty when unset.** Whitelist and subprocess settings become effective engine settings with this change. Deploy the environment settings together with the code.

To retain the selected production compatibility configuration, configure each applicable deployment module with:

```bash
BKAPP_SOPS_MAKO_IMPORT_MODULES=datetime,datetime.datetime,datetime.date,re,hashlib,random,time,os.path,config.mock.mock_json:json
...
Merge any existing application imports into the table. Apply the settings to Web, task-executing Workers and any other modules that render expressions, then restart those processes; deployment variables are scoped per module. This preset retains unconditional security checks and allows unsupported-context fallback, but never falls back on infrastructure failures such as startup errors or timeouts. `NO_NETWORK=0` does not require network namespace isolation. This deployment example does not change the defaults listed below.
...
Administrators can run the inventory command on demand; deployment does not start a scan. It exports expressions, rule findings and template links, with execution statistics. It reads the database without executing business expressions.

```bash
python manage.py export_mako_expression_inventory --tenant-id TENANT_ID --output /tmp/mako_inventory.csv --hits-only
```

The multi-tenant command requires `--tenant-id`; templates, business data, execution records and scheduled-task queries are restricted to that tenant, which is also included in the CSV.
