...
- `name`：标准插件名称。
- `code`：标准插件唯一编码，请保持 `code` 和 `version` 全局了联合唯一。
...
有时候我们需要对某个插件进行升级操作，例如为表单中添加新的字段，为后台逻辑添加新的功能，那么这个时候就需要修改现有插件的逻辑；但是我们现有的流程中可能有大量的存量模板和任务使用到了这个插件，直接修改插件的代码可能会导致现有的模板和任务不可用，所以正确的做法应该是为这个插件增加一个新的版本。

通过设置 Component 的 `version` 类属性，我们能够为 `code` 相同的插件添加不同版本的插件，以保证插件的功能升级不会影响用户的正常使用，用户只需要在合适的时候将旧的插件升级到新版本即可。
