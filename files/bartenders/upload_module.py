...
class UploadModuleBartender(UploadRequestBartender):
    def process_request(self, request, *args, **kwargs):
        ...
        if not file_name:
            logger.error(f"文件上传失败: 文件名[{file_name}]不符合要求, 请修改后重试 | process_request")
            return {
                "result": False,
        ...
        if INVALID_CHAR_REGEX.findall(file_name):
            logger.error(f'文件上传失败: 文件名{file_name}不能包含\\/:*?"<>|等特殊字符 | process_request')
            return {
                "result": False,
                "message": _(f'文件上传失败，文件名{file_name}不能包含\\/:*?"<>|等特殊字符 | process_request'),
                "code": 400,
            }

        if not file_path:
            logger.error(f"文件上传失败: 文件路径[{file_path}]不符合要求, 请修改后重试 | process_request")
            return {
                "result": False,
        ...
        try:
            ...
            logger.error(f"文件上传失败: 文件归档失败请重试, 如持续失败可联系管理员处理{e} | process_request")
            return {"result": False, "message": _("文件上传失败: 文件归档失败请重试, 如持续失败可联系管理员处理 | process_request"), "code": 500}
