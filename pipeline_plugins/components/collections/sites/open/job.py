...
class JobFastExecuteSQLService(JobService):
    def execute(self, data, parent_data):
        executor = parent_data.get_one_of_inputs('executor')
        biz_cc_id = parent_data.get_one_of_inputs('biz_cc_id')
        client = get_client_by_user(executor)
...
class JobCrontabTaskComponent(Component):
    name = _(u'新建定时作业')
    code = 'job_crontab_task'
    bound_service = JobCrontabTaskService
    form = '%scomponents/atoms/sites/%s/job/job_crontab_task.js' % (settings.STATIC_URL, settings.RUN_VER)
        original_ip_list = data.get_one_of_inputs('job_ip_list')
        ip_info = cc_get_ips_info_by_str(
            username=executor,
            biz_cc_id=biz_cc_id,
    ...
    def schedule(self, data, parent_data, callback_data=None):
        return super(JobFastExecuteSQLService, self).schedule(data, parent_data, callback_data)

    def outputs_format(self):
        return super(JobFastExecuteSQLService, self).outputs_format()


class JobFastExecuteSQLComponent(Component):
    name = _(u'快速执行SQL脚本')
