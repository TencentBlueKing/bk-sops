    ...
    def inputs_format(self):
        return [
            ...
            self.InputItem(
                ...
                key="job_ip_list",
                type="string",
                schema=StringItemSchema(description=_("执行脚本的目标机器 IP 或 host_id，多个用英文逗号 `,` 分隔")),
            ),
            self.InputItem(
                name=_("目标账户"),
                key="job_account",
...
class JobFastExecuteScriptComponent(Component):
    ...
    desc = _(
        ...
        "插件版本V2.0支持在目标IP参数中传入host_id，插件会自动识别并构造相应的调用参数。\n"
        "插件版本V2.1支持使用节点名称+时间戳作为JOB任务名。\n"
        "注：插件版本v1.1中跨业务执行脚本时需要在作业平台添加白名单。\n"
        "注：插件版本v1.2及以上中滚动执行要求作业平台版本>=V3.6.0.0。\n"
        "注：插件版本v1.2及以上中超时限制[1, 259200]要求作业平台版本>=V3.7.5。\n"
    )
