...
from pipeline_plugins.components.collections.sites.open.job.base import Jobv4ScheduleService
...
class JobFastPushFileService(Jobv4ScheduleService, GetJobTargetServerMixin):
    ...
    def inputs_format(self):
        return [
            ...
            self.InputItem(
                ...
                key="job_ip_list",
                type="string",
                schema=StringItemSchema(description=_("文件分发目标机器 IP 或 host_id，多个用英文逗号 `,` 分隔")),
            ),
            self.InputItem(
                name=_("目标账户"),
                key="job_account",
    ...
    def plugin_execute(self, data, parent_data):
        ...
        for attr in attr_list:
            ...
            clean_result, target_server = self.get_target_server_hybrid_with_host_id(
            ...
            if job_rolling_execute:
                # 滚动机制为4时（不依赖于job方实行滚动），不传rolling_config
                if job_rolling_mode != 4:
        ...
        job_instance_id_list, job_inst_name = [], []
        data.outputs.requests_error = ""

        # 初始化重试状态存储（供基类 plugin_schedule 使用）
        ...
        for index, res in enumerate(job_result_list):
            ...
            if job_result["result"]:
                ...
                job_instance_id_list.append(job_id)

                data.outputs._original_params_map[job_id] = params_list[index]
                data.outputs._retry_count_map[job_id] = 0
    ...
    def outputs_format(self):
        return [
            ...
            self.OutputItem(
                name=_("分发成功数"), key="success_count", type="string",
...
class JobFastPushFileComponent(Component):
    ...
    form = "%scomponents/atoms/job/fast_push_file/v4_0.js" % settings.STATIC_URL
    version = "v4.0"
    desc = _(
        ...
        "5. V2.1及以上中超时限制[1, 259200]要求作业平台版本>=V3.7.5。\n"
        "6. V3.0版本支持在目标IP参数中传入host_id，插件会自动识别并构造相应的调用参数。\n"
        "7. V3.1版本支持使用节点名称+时间戳作为 job 任务名，便于在作业平台中区分任务来源。\n"
        "8. V4.0版本支持滚动机制按失败ip重试三次。\n"
    )
