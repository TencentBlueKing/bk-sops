...
class BasePluginService(Service):
    ...
    def __init__(self, *args, **kwargs):
        super(BasePluginService, self).__init__(*args, **kwargs)
        self._decrypted_values = {}  # 用于存储解密后的值
    ...
    def get_decrypted_values(self, input_key):
        """
        获取解密后的值

        :param input_key: 插件输入对象key值
        :return: 解密后的值
        """
        return self._decrypted_values.get(input_key)

    def set_decrypted_values(self, input_key, decrypted_values):
        """
        设置解密后的值

        :param input_key: 插件数据对象
        :param decrypted_values: 解密后的值
        """
        # 将解密后的值存储在独立的中间变量中，避免污染 outputs
        self._decrypted_values[input_key] = decrypted_values
    ...
    def _auto_decrypt_password_inputs(self, data):
        ...
        for key, value in list(inputs.items()):
            # 这里特殊处理schedule的逻辑，存在execute执行完之后，密码字段被掩码了，需要这里恢复
            # 首先判断是否有解密过，如果有的话直接使用，没有则解密
            decrypted_values = self.get_decrypted_values(key)
            if decrypted_values is not None:
                plain = decrypted_values
            else:
            ...
            self.set_decrypted_values(key, plain)  # 记录解密后的值用于轮询那边调用
