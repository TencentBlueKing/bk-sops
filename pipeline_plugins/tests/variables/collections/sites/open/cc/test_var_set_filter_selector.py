...
class VarSetFilterSelectorTestCase(TestCase):
    def setUp(self):
        ...
        self.input_output_success_return = SetInfo([{"bk_set_name": "set", "bk_set_id": 1}])

        self.get_set_info_fail_return = SetInfo([])
    ...
    def test_get_set_info_fail_case(self, mock_get_client_by_user_return):
        ...
        self.SetInfoEqual(set_module_selector.get_value(), self.get_set_info_fail_return)

    def SetInfoEqual(self, first_inst, second_inst):
        """
        自定义断言：用于判断两个对象的属性值是否相等
        """
        if not (first_inst.bk_set_name == second_inst.bk_set_name and first_inst.bk_set_id == second_inst.bk_set_id):
