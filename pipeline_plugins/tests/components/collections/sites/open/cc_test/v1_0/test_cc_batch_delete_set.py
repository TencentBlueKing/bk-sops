...
from pipeline_plugins.components.collections.sites.open.cc_batch_delete_set.v1_0 import (
    CCBatchDeleteSetComponent
)


class CCBatchDeleteSetComponentTest(TestCase, ComponentTestMixin):
    def component_cls(self):
        return CCBatchDeleteSetComponent
...
GET_CLIENT_BY_USER = 'pipeline_plugins.components.collections.sites.open.cc_batch_delete_set.v1_0.get_client_by_user'
...
SELECT_BY_TEXT_ERROR_LEVEL_FAIL_CASE = ComponentTestCase(
    ...
    execute_assertion=ExecuteAssertion(
        success=False, outputs={
            'ex_data': u'输入文本路径[蓝鲸>Yun]与业务拓扑层级不匹配'
        }),
    schedule_assertion=None,
    execute_call_assertion=None,
    patchers=[
