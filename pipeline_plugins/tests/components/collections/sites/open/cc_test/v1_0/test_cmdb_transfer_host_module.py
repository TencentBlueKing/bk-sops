...
GET_CLIENT_BY_USER = (
    "pipeline_plugins.components.collections.sites.open.cc_transfer_host_module.v1_0" ".get_client_by_user"
)
...
CC_GET_HOST_ID_BY_INNERIP = (
    "pipeline_plugins.components.collections.sites.open.cc_transfer_host_module.v1_0" ".cc_get_host_id_by_innerip"
)
CC_LIST_MATCH_NODE_INST_ID = (
    "pipeline_plugins.components.collections.sites.open.cc_transfer_host_module.v1_0" ".cc_list_match_node_inst_id"
)
...
SELECT_BY_TEXT_ERROR_PATH_FAIL_CASE = ComponentTestCase(
    ...
    parent_data=COMMON_PARENT,
    execute_assertion=ExecuteAssertion(success=False, outputs={"ex_data": u"不存在该拓扑路径：蓝鲸>Yun>set>module"}),
    schedule_assertion=None,
    execute_call_assertion=[
        CallAssertion(func=CC_GET_HOST_ID_BY_INNERIP, calls=[Call("admin", 2, ["1.1.1.1", "2.2.2.2"], 0)])
    ],
...
SELECT_BY_TEXT_ERROR_LEVEL_FAIL_CASE = ComponentTestCase(
    ...
    parent_data=COMMON_PARENT,
    execute_assertion=ExecuteAssertion(success=False, outputs={"ex_data": u"输入文本路径[蓝鲸>Yun>module]与业务拓扑层级不匹配"}),
    schedule_assertion=None,
    execute_call_assertion=[
        CallAssertion(func=CC_GET_HOST_ID_BY_INNERIP, calls=[Call("admin", 2, ["1.1.1.1", "2.2.2.2"], 0)])
    ],
