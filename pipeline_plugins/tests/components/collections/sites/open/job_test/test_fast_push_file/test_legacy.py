...
from pipeline_plugins.components.collections.sites.open.job.fast_push_file.legacy import JobFastPushFileComponent
...
GET_CLIENT_BY_USER = (
    "pipeline_plugins.components.collections.sites.open.job.fast_push_file.legacy.get_client_by_username"
)
...
GET_JOB_INSTANCE_URL = (
    "pipeline_plugins.components.collections.sites.open.job.fast_push_file.legacy.get_job_instance_url"
)
JOB_HANDLE_API_ERROR = (
    "pipeline_plugins.components.collections.sites.open.job.fast_push_file.legacy.job_handle_api_error"
)
GET_NODE_CALLBACK_URL = (
    "pipeline_plugins.components.collections.sites.open.job.fast_push_file.legacy.get_node_callback_url"
)
...
def PUSH_FILE_SUCCESS_CASE():
    return ComponentTestCase(
        name="fast_push_files legacy success",
        inputs=INPUT,
...
def PUSH_FILE_FAIL_CASE():
    return ComponentTestCase(
        name="fast_push_files legacy fail case",
        inputs=INPUT,
        parent_data={"executor": "executor", "project_id": "project_id", "biz_cc_id": 1, "tenant_id": "system"},
        execute_assertion=ExecuteAssertion(
            success=False,
