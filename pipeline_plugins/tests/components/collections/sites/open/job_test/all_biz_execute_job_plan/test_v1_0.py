...
EXECUTE_JOB_PLAN_SUCCESS_CASE = ComponentTestCase(
    ...
    inputs={
        'all_biz_cc_id': 2,
        'job_plan_id': 1000010,
        'job_global_var': [{'id': 1000030, 'type': 1, 'name': 'name', 'value': 'test', 'description': ''},
...
INVALID_IP_FAIL_CASE = ComponentTestCase(
    ...
    execute_assertion=ExecuteAssertion(success=False, outputs={
        'ex_data': 'IP 校验失败，请确认输入的 IP 0:192.168.20.256 是否合法'
    }),
...
EXECUTE_JOB_PLAN_CALL_FAIL_CASE = ComponentTestCase(
    ...
    execute_assertion=ExecuteAssertion(success=False, outputs={
        'ex_data': (
            '调用作业平台(JOB)接口jobv3.execute_job_plan返回失败, params={params}, error={error}, request_id={request_id}').format(
            params=json.dumps({
                'bk_biz_id': 999101,
                'job_plan_id': 1000010,
                'global_var_list': [
...
INVALID_CALLBACK_DATA_CASE = ComponentTestCase(
    ...
    execute_call_assertion=[
        CallAssertion(func=INVALID_CALLBACK_DATA_CLIENT.jobv3.execute_job_plan, calls=[Call({
            'bk_biz_id': 2,
            'job_plan_id': 1000010,
            'global_var_list': [
                {
