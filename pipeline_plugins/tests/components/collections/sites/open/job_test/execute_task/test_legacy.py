...
EXECUTE_JOB_FAIL_CASE = ComponentTestCase(
    ...
    execute_assertion=ExecuteAssertion(
        ...
        outputs={
            "ex_data": (
                "调用作业平台(JOB)接口jobv3.execute_job_plan返回失败, error=message token, params={params}"
            ).format(
                params=json.dumps(
                    {
                        "bk_scope_type": "biz",
...
JOB_EXECUTE_NOT_SUCCESS_CASE = ComponentTestCase(
    ...
    schedule_assertion=ScheduleAssertion(
        ...
        outputs={
            ...
            "client": JOB_EXECUTE_NOT_SUCCESS_CLIENT,
            "ex_data": {
                "exception_msg": ("任务执行失败，<a href='%s' target='_blank'>" "前往作业平台(JOB)查看详情</a>")
                % "instance_url_token",
                "task_inst_id": 56789,
                "show_ip_log": True,
            },
...
INVALID_IP_CASE = ComponentTestCase(
    ...
    execute_assertion=ExecuteAssertion(
        success=False,
        outputs={
            "ex_data": "无法从配置平台(CMDB)查询到对应 IP，请确认输入的 IP 是否合法。查询失败 IP： 1.1.1.1,2.2.2.2"
        },
    ),
    schedule_assertion=None,
    execute_call_assertion=[
