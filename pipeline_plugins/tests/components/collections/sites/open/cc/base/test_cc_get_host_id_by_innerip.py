...
class CCGetHostIdByInnerIpTestCase(TestCase):
    ...
    def test__get_business_host_return_empty(self):
        ...
        self.assertEqual(data["message"], f"IP [{self.ip_list}] 在本业务下不存在: 请检查配置, 修复后重新执行任务")
    ...
    def test__return_host_list_gt_ip_list(self):
        ...
        self.assertFalse(data["result"])
        self.assertEqual(data["message"], "IP [['1.1.1.1', '2.2.2.2']] 在本业务下重复: 请检查配置, 修复后重新执行")
    ...
    def test__return_host_list_lt_ip_list(self):
        ...
        self.assertFalse(data["result"])
        self.assertEqual(data["message"], "IP [{'3.3.3.3'}] 在本业务下不存在: 请检查配置, 修复后重新执行")
