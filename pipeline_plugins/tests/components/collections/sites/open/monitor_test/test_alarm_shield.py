...
class MonitorAlarmShieldComponentTest(TestCase, ComponentTestMixin):
    def cases(self):
        return [SHIELD_FAIL_CASE, SHIELD_SUCCESS_CASE]
...
GET_SET_LIST = "pipeline_plugins.components.collections.sites.open.monitor.alarm_shield.v1_0.get_set_list"
GET_LIST_BY_SELECTED_NAMES = (
    "pipeline_plugins.components.collections.sites.open.monitor.alarm_shield.v1_0" ".get_list_by_selected_names"
)
GET_SERVICE_TEMPLATE_LIST = (
    "pipeline_plugins.components.collections.sites.open.monitor.alarm_shield.v1_0" ".get_service_template_list"
)
GET_SERVICE_TEMPLATE_LIST_BY_NAMES = (
    "pipeline_plugins.components.collections.sites.open.monitor.alarm_shield.v1_0" ".get_service_template_list_by_names"
)

# mock client
ADD_SHIELD_FAIL_CLIENT = MockClient(add_shield_result={"result": False, "message": "create shield fail"})

ADD_SHIELD_SUCCESS_CLIENT = MockClient(add_shield_result={"result": True, "data": {"id": "1"}, "message": "success"})
...
SHIELD_FAIL_CASE = ComponentTestCase(
    ...
    execute_assertion=ExecuteAssertion(
        ...
        outputs={
            "shield_id": "",
            "message": '调用监控平台(Monitor)接口monitor.add_shield返回失败, params={"begin_time":"2020-09-28 11:18:58",'
    ...
    patchers=[
        Patcher(target=GET_CLIENT_BY_USER, return_value=ADD_SHIELD_FAIL_CLIENT),
...
SHIELD_SUCCESS_CASE = ComponentTestCase(
    ...
    patchers=[
        Patcher(target=GET_CLIENT_BY_USER, return_value=ADD_SHIELD_SUCCESS_CLIENT),
        Patcher(target=GET_SET_LIST, side_effect=get_set_list),
        Patcher(target=GET_LIST_BY_SELECTED_NAMES, side_effect=get_list_by_selected_names),
        Patcher(target=GET_SERVICE_TEMPLATE_LIST, side_effect=get_service_template_list),
        Patcher(target=GET_SERVICE_TEMPLATE_LIST_BY_NAMES, side_effect=get_service_template_list_by_names),
