...
def cmdb_search_host(request, bk_biz_id, bk_supplier_account="", bk_supplier_id=0):
    ...
    condition = [{"bk_obj_id": "host", "fields": []}]
    if "set" in fields:
        condition.append({"bk_obj_id": "set", "fields": []})
    if "module" in fields:
        condition_module = {
            "bk_obj_id": "module",
            "fields": [],
    ...
    kwargs = {"bk_biz_id": bk_biz_id, "bk_supplier_account": bk_supplier_account, "condition": condition}
    if request.GET.get("topo", None):
        ...
        if "module" in fields:
            condition_module["condition"] = [{"field": "bk_module_id", "operator": "$in", "value": topo_modules_id}]
        else:
            kwargs["condition"].append(
                {
                    "bk_obj_id": "module",
                    "fields": ["bk_module_id", "bk_module_name"],
                    "condition": [{"field": "bk_module_id", "operator": "$in", "value": topo_modules_id}],
                }
            )
    ...
    if host_info:
        ...
        if "agent" in fields:
            agent_kwargs = {
                ...
                "hosts": [
                    {"bk_cloud_id": get_bk_cloud_id_for_host(host, "cloud"), "ip": host["bk_host_innerip"]}
                    for host in data
                ],
            }
