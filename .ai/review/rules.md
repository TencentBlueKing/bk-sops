# bk-sops release_humming_bird 审查规则

本规则适用于 PR 目标分支 `release_humming_bird`，须与本次 base/head 和有效依赖一起使用。
...
release_humming_bird 本基线不是多租户研发线；不要假设 Project/IAM 已带租户参数，也不要对与租户无关的小修复强制套用不存在的模型/测试。需要跨线回移时明确引入能力和迁移范围。
...
- API 变更同步检查双语文档的功能、参数表、请求/响应示例和说明，以及 `gcloud/apigw/management/commands/data/api-resources.yml` 的参数、请求体和响应 schema。当前版本为 `swagger: '2.0'`，使用 Swagger 2.0 的 body parameter（`in: body`）及 responses的表达方式；不为补 schema 擅自要求整个资源版本升级。
- 文档位置为 `docs/zh_hans/apidoc/` / `docs/en/apidoc/`，实际归档由 config/default.py 选择 `gcloud/apigw/docs/apigw-docs.tgz`，正文内部为 zh/en；结合源码与配置核对消费产物，不硬编码不存在的根 apigw/MCP YAML，也不因另一后缀存在就要求同步生成两份。
