# bk-sops V3.26_lts 审查规则

本规则适用于 PR 目标分支 `V3.26_lts`，须与本次 base/head 和有效依赖一起使用。
...
- 文档位置为 `docs/apidoc/zh_hans/` / `docs/apidoc/en/`，实际归档由 config/default.py 选择 `gcloud/apigw/docs/apigw-docs.tgz`，正文内部为 zh/en；结合源码与配置核对消费产物，不硬编码不存在的根 apigw/MCP YAML，也不因另一后缀存在就要求同步生成两份。
