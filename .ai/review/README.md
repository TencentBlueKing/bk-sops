...
工作流、`knowledge.md` 和 `rules.md` 合入目标分支 `dev_multi_tenant` 后，针对该目标分支的 PR 在 opened、synchronize、reopened 或 ready_for_review 事件触发时才会使用本方案。仅把文件提交到接入 PR 的 head，或只合入 master，不代表本目标分支已经接入。既有 PR 需要在目标分支合入后再次产生上述事件；本次文档和静态检查不是线上审查成功的证据。
