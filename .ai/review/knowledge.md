# bk-sops V3.26_lts 审查知识库

## 分支基线

- 核验日期：2026-09-09；目标 `V3.26_lts`；base SHA `a6eccfa1c4b9dce4e212eced9e1010d86ba6d317`。这是独立维护分支的快照，不是 master 的能力清单。
- 运行时：`python-3.6.12`；依赖：`Django==3.2.15`、`bamboo-pipeline==3.23.1`、`celery==4.4.0`、`blueapps[opentelemetry]==4.4.7`。
...
V3.26 的 create_task 使用本分支 jsonschema/validator 和 create_method="api" 路径，锁定 bamboo-pipeline 3.23.1；V3.28 或新版引擎的字段/修复不得默认为已包含。
