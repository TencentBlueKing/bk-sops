# bk-sops dev_multi_tenant_0729_merge 审查知识库

## 分支基线

- 核验日期：2026-09-09；目标 `dev_multi_tenant_0729_merge`；base SHA `156ee10b4882fb854789b306260e595f2aae2b8a`。这是独立维护分支的快照，不是 master 的能力清单。
...
这是独立保留的集成目标：2026-08-24 仍接收 PR #8489/#8491，head 并非 dev_multi_tenant 的祖先（本次刷新左右独有提交 8/20）。与 dev_multi_tenant 不等价，尤其较新的插件网关禁用保护和测试未必在此分支存在；按本 head 核对，不宣称已全部合并。`gcloud/apigw/docs/` 同时有 .tgz/.tar.gz，按 definition 核对实际使用产物。
