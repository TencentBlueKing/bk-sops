...
class BKIAMClient(object):
    ...
    def _perm_api_url(self, path):
        return self._request_url(path='perm/systems/{system_id}/{path}'.format(
            system_id=self.system_id,
            path=path.lstrip('/'))
        )

    def _resource_api_url(self, path):
        return self._request_url(path='perm/systems/{system_id}/resources/{path}'.format(
            system_id=self.system_id,
            path=path.lstrip('/'))
        )

    def _perm_model_api_url(self, path):
        return self._request_url(path='perm-model/systems/{system_id}/{path}'.format(
            system_id=self.system_id,
            path=path.lstrip('/'))
        )
    ...
    def _request(self, method, url, params=None, data=None, headers=None):
        ...
        try:
            ...
            if not resp_data['result']:
                logger.error(
                    'bk_iam return error, message {message}, request_id={request_id}, '
    ...
    def batch_verify_combine_resources_perms(
            self,
            principal_type,
            principal_id,
            scope_type,
    ...
    def update_resource(self, scope_type, scope_id, resource_type, resource_id, resource_name, properties=None):
        """更新资源实例
