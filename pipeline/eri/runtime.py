...
class BambooDjangoRuntime(
    ...
    def _prepare(
        self, pipeline: dict, root_id: str, parent_id: Optional[str] = None
        ...
        # collect all node outputs and initial refrence
        ...
        for act in pipeline["activities"].values():
            if act["type"] == NodeType.ServiceActivity.value:
                ...
                sub_nodes, sub_datas, sub_ctx_values, sub_ctx_outputs = self._prepare(
                    pipeline=act["pipeline"], root_id=root_id, parent_id=act["id"]
                )
        ...
        for key, references in context_var_references.items():
            ...
            while queue:
                ...
                # processed
                if r in final_refrences[key]:
                    continue

                final_refrences[key].add(r)
    ...
    def prepare_run_pipeline(self, pipeline: dict, root_pipeline_data: dict, **options) -> int:
        """
        ...
        nodes, datas, context_values, context_outputs = self._prepare(pipeline=pipeline, root_id=pipeline["id"])
        datas.append(
            Data(
                node_id=pipeline_id,
                inputs=json.dumps({k: {"need_render": False, "value": v} for k, v in root_pipeline_data.items()}),
                outputs="{}",
            )
        )
