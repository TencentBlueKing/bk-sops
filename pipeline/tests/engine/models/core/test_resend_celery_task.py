...
from pipeline.engine.models import ResendCeleryTask


class ResendCeleryTaskTestCase(TestCase):
    def tearDown(self):
        ResendCeleryTask.objects.all().delete()

    def test_kwargs_dict(self):
        kwargs = {"task_kwargs": "token"}
        task = ResendCeleryTask.objects.create(
            name="name",
    ...
    def test_resend__empty_send_error(self):
        ...
        with patch("pipeline.engine.models.core.current_app", current_app):
            self.assertEqual(task.exec_trace, None)
            task.resend()
            self.assertNotEqual(task.exec_trace, None)
            self.assertEqual(task.resend_times, 1)
    ...
    def test_resend__process_send_error(self):
        ...
        task = ResendCeleryTask(
            name="name",
            kwargs=json.dumps({"task_kargs": "token"}),
            type=ResendCeleryTask.TASK_TYPE_PROCESS,
    ...
    def test_resend__node_send_error(self):
        ...
        task = ResendCeleryTask(
            name="name",
            kwargs=json.dumps({"task_kargs": "token"}),
            type=ResendCeleryTask.TASK_TYPE_NODE,
    ...
    def test_resend__schedule_send_error(self):
        ...
        task = ResendCeleryTask(
            name="name",
            kwargs=json.dumps({"task_kargs": "token"}),
            type=ResendCeleryTask.TASK_TYPE_SCHEDULE,
    ...
    def test_record__with_dict_kwargs(self):
        ...
        task = ResendCeleryTask.objects.record(name, kwargs, type, extra_kwargs)
    ...
    def test_record__with_str_kwargs(self):
        ...
        task = ResendCeleryTask.objects.record(name, json.dumps(kwargs), type, json.dumps(extra_kwargs))
        self.assertEqual(task.name, name)
        self.assertEqual(task.kwargs_dict, kwargs)
        self.assertEqual(task.type, type)
        self.assertEqual(task.extra_kwargs_dict, extra_kwargs)
