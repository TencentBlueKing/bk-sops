...
class TestStreamValidation(TestCase):
    ...
    def test_match_converge(self):
        for n, i in enumerate(gateway_valid_cases, start=1):
            ...
            converge_id, _ = match_converge(converges=converge,
                                            gateways=gateway,
                                            cur_index=start,
            ...
            for _, g in gateway.items():
                ...
                self.assertEqual(actual,
                                 expect,
                                 msg='{id} actual: {a}, expect: {e}'.format(id=g[PE.id], a=actual, e=expect))
        ...
        for n, i in enumerate(gateway_invalid_cases, start=1):
            ...
            try:
                ...
                self.assertEqual(actual,
                                 expect,
                                 msg='invalid assert{id} actual: {a}, expect: {e}'.format(id=n, a=actual, e=expect))
    ...
    def test_validate_stream(self):
        ...
        for nid, expect in stream_assert.items():
            ...
            self.assertEqual(
                actual,
                expect,
                msg='{id} actual: {a}, expect: {e}'.format(id=nid, a=actual, e=expect))
